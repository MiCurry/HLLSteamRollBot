...
class PlayerTeamConfidence(Enum):
    STRONG = "strong"
    MIXED = "mixed"
...
def detect_team(player) -> Tuple[Team, PlayerTeamAssociation]:
    def get_value(item):
        return item[1]
    ...
    if len(player['weapons']) > 0:
        for weapon in sorted(player['weapons'].items(), key=get_value, reverse=True):
            if WEAPON_SIDE_MAP.get(weapon[0]) == Team.ALLIES:
                allies_count += weapon[1]
            elif WEAPON_SIDE_MAP.get(weapon[0]) == Team.AXIS:
                axis_count += weapon[1]

    if len(player['death_by_weapons']) > 0:
        for weapon in sorted(player['death_by_weapons'].items(), key=get_value, reverse=True):
    ...
    if axis_count == 0 and allies_count == 0:
        return Team.UNKNOWN, PlayerTeamAssociation(side=Team.UNKNOWN, confidence=PlayerTeamConfidence.STRONG, ratio=0)
